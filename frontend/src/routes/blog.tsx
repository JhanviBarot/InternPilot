...
export const Route = createFileRoute("/blog")({
  head: () => ({
    meta: [
      { title: "How a WhatsApp Rant Turned Into InternPilot - InternPilot Blog" },
      {
        name: "description",
        content:
          "The honest story of how InternPilot got built in seven days - ghost jobs, deadlocks, things we cut, and a friend's rejection email that started it all.",
      },
      { property: "og:title", content: "How a WhatsApp Rant Turned Into InternPilot" },
      {
        property: "og:description",
        content:
          "One annoyed message in an old school group chat. Seven days. The honest story behind InternPilot.",
      },
    ],
  }),
  component: BlogPost,
...
const SECTIONS: { heading: string; body: string }[] = [
  {
    heading: "The first plan, and why it was wrong",
    body: `The AI Club dropped the theme list for the 7-day buildathon a few weeks later, and like pretty much everyone in our batch, we spent a first day going back and forth on what to build. We had a Notion page (badly organised, half the entries crossed out by the next morning) with ideas ranging from a GPA calculator nobody had asked for to some kind of assignment helper, paper analyzer, paper predictor and all such kinda things.
...
That conversation is the actual reason InternPilot exists in the form it does. Not "more listings." Filtering and ranking the listings that already exist, so a student isn't drowning in them.`,
  ...
  {
    heading: "What broke first, once we actually looked",
    body: `Once we started pulling real postings to test the ingestion code, one of us started manually spot-checking a handful of them out of curiosity more than anything else. Several looked completely normal - real company name, specific-sounding role, a reasonably detailed description - but searching around for any sign the company was actually hiring for that role right now turned up nothing. No recent activity. Postings that had clearly been live for sixty, ninety days, still sitting there, still being served to anyone searching.
...
That became its own module, separate from matching entirely - score every posting for "is this even real" before it's allowed to show up in a feed at all.`,
  ...
  {
    heading: "Building a profile that isn't just a PDF",
    body: `The harder problem, honestly, was the other side - representing the student, not the listing. A resume is text. It doesn't tell software that "built a pipeline using Pandas" and a job asking for "data manipulation experience" mean almost the same thing, because they don't share a single word in common.
...
We picked a small model that runs locally instead of calling an API for every embedding - partly because we were burning through free-tier limits fast enough already, and partly because there's something nice about not depending on a network call just to compare two pieces of text. The first time we ran it against an actual resume - one of our own, not a sample one - and saw it correctly line up "built data pipelines" with a posting asking for "ETL experience," neither of which share a word, that was the point it stopped feeling like a checkbox feature and started feeling like the thing the whole project was actually about.`,
  ...
  {
    heading: "When the AI started making things up",
    body: `This is the part that worried us more than any bug did.
...
It cost us a chunk of an evening we'd planned to spend on the frontend instead. But shipping a tool that quietly invents things on your resume felt like a worse outcome than being a few hours behind.`,
  ...
  {
    heading: "The unglamorous stuff that almost broke the demo",
    body: `None of this is the exciting part of the writeup, but it's the part that decides whether your demo actually runs on the day.
...
We picked a threshold above which a posting gets flagged as likely dead, and tuned it against listings we'd manually checked by hand. It's not a perfect model. It's a rule-based one, deliberately - no LLM call needed to compute it, which also meant it was fast enough to run on every single posting before it's allowed into anyone's feed, instead of being a slow afterthought.`,
  ...
  {
    heading: "The cohort thing nobody asked us to build",
    body: `This is the feature neither of us had planned, and it came out of looking at our own test data more than any grand insight.
...
So once enough applications have gone to the same company - we picked five as the threshold, since anything smaller felt too noisy to trust - the aggregate response rate starts feeding directly into how that company's postings get ranked. Not who applied, not what they sent, just the count and the outcome. A posting can look like a ninety-percent semantic match and still drop in the feed with a plain note explaining why: the company's last several applicants never heard back, regardless of how strong they looked on paper.`,
  ...
  {
    heading: "Grading ourselves honestly",
    body: `The part we're most careful about, because it would be very easy to fudge, is whether any of this is actually working.
...
That one stung a bit, not going to lie. But the senior who told us to scope down hard was right. Shipping fewer things that are properly checked beats shipping more things that might fall over during a demo.`,
  ...
  {
    heading: "The day it almost fell apart",
    body: `Somewhere around day five, with most of the core pieces working individually, we tried running the whole thing end to end for the first time - signup through to seeing a ranked feed - and it just hung. Not an error, not a crash. It sat there loading.
...
function BlogPost() {
  return (
    <div className="min-h-screen">
      <Nav />
      <main className="mx-auto max-w-3xl px-6 pt-20 pb-40">
        ...
        <motion.h1
          ...
          className="mt-6 font-display text-4xl sm:text-5xl md:text-6xl font-medium leading-[1.05] tracking-tight text-balance"
        >
        ...
        <motion.div
          ...
          className="mt-10 origin-left h-px w-full"
        ...
        >
          <Prose>
            <p>
              It didn't start as a project. It started as a guy in a school friend group chat being
              annoyed about a rejection email.
            </p>
            <p>
              This was sometime before the AI Club had even put out the buildathon themes. We were
              both home for the break, doing the usual nothing-in-particular thing you do on vacation
              - half-watching something, half on the phone. A friend from school - not someone at
              DAU, he's at a different college entirely, we just never left the group chat that's been
              running since class 12 - had just gotten a rejection mail for something he'd applied to
              that same afternoon. Not the next day. The same afternoon. He sent a screenshot with one
              line: <em>"bro didn't even let it sit overnight."</em>
            </p>
            <p>
              Nobody really replied properly. A couple of laughing emojis, somebody said "HR be like
              that," and the chat moved on to something else entirely. That's usually how these things
              go - everyone's been there, everyone vents for two minutes, and then everyone goes back
              to whatever they were half-watching.
            </p>
            <p>I didn't do anything about it at the time. It just sat there.</p>
          </Prose>
        </motion.div>
        ...
        {SECTIONS.map((s, i) => (
          ...
          >
            <h2
              className="mt-16 mb-6 font-display text-2xl sm:text-3xl font-medium tracking-tight"
            >
              {s.heading}
            </h2>
            <Prose>
        ...
        >
          <span className="italic">- built for the DAU AI Club buildathon</span>
          <Link
            to="/onboarding"
            className="inline-flex items-center gap-1.5 rounded-full px-4 py-2 text-sm font-medium transition-transform hover:-translate-y-0.5"
            style={{
...
function Prose({ children }: { children: React.ReactNode }) {
  return (
    <div
      className="space-y-5 text-base md:text-[17px] leading-[1.75] text-foreground/85"
    >
      {children}
    </div>
  );
